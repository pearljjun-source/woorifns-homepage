...
const NOTIFY_EMAIL = "stormroller@hanmail.net";
...
interface InquiryPayload {
  ...
  record: {
    ...
    breakfast_per_week: number;
    lunch_per_week: number;
    dinner_per_week: number;
    message: string | null;
    created_at: string;
  };
}
...
serve(async (req) => {
  try {
    ...
    const mealInfo = [
      record.breakfast_per_week > 0 ? `조식 ${record.breakfast_per_week}회` : "",
      record.lunch_per_week > 0 ? `중식 ${record.lunch_per_week}회` : "",
      record.dinner_per_week > 0 ? `석식 ${record.dinner_per_week}회` : "",
    ]
      .filter(Boolean)
      .join(" / ");
    ...
    const emailHtml = `
      ...
      <table style="border-collapse:collapse;width:100%;max-width:600px;">
        ...
        <tr><td style="padding:8px;border:1px solid #ddd;font-weight:bold;">주간 식사</td><td style="padding:8px;border:1px solid #ddd;">${mealInfo || "미선택"}</td></tr>
